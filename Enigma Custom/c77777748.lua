...
function c77777748.ctop(e,tp,eg,ep,ev,re,r,rp)
	e:GetHandler():AddCounter(0x666+COUNTER_NEED_ENABLE,1)
end
...
function c77777748.target(e,tp,eg,ep,ev,re,r,rp,chk)
	local b1=e:GetHandler():IsCanRemoveCounter(tp,0x666,2,REASON_COST)
		and Duel.IsExistingMatchingCard(c77777748.filter2,tp,LOCATION_REMOVED,0,1,nil)
	local b2=e:GetHandler():IsCanRemoveCounter(tp,0x666,25,REASON_COST)
	...
	if op==0 then
		...
		e:GetHandler():RemoveCounter(tp,0x666,2,REASON_COST)
		Duel.SetOperationInfo(0,CATEGORY_TODECK,nil,1,tp,LOCATION_REMOVED)
	else
		e:SetCategory(CATEGORY_DESTROY)
		e:GetHandler():RemoveCounter(tp,0x666,25,REASON_COST)
