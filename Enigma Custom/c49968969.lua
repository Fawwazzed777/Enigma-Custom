...
function s.filter(c)
	return c:IsFaceup() and c:IsSetCard(0x45) and c:IsType(TYPE_XYZ)
end
...
function s.mattg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.mfilter,tp,LOCATION_GRAVE,LOCATION_GRAVE,1,nil) end
end
function s.matop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,s.mfilter,tp,LOCATION_GRAVE,LOCATION_GRAVE,1,1,nil)
	if #g>0 then
		Duel.Overlay(ec,g)
	end
end
