...
function s.initial_effect(c)
	...
	e4:SetTarget(s.applytg)
	e4:SetOperation(s.applyop)
	c:RegisterEffect(e4)
end
...
function s.exname(c,code)
	return c:IsCode(code) and (c:IsFaceup() or c:IsLocation(LOCATION_GRAVE))
end
function s.thfilter(c,tp)
	return c:IsSetCard(0x7f3) and c:IsAbleToHand() 
		and not Duel.IsExistingMatchingCard(s.exname,tp,LOCATION_ONFIELD+LOCATION_GRAVE,0,1,nil,c:GetCode())
end
...
function s.applytg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.applyfilter,tp,LOCATION_GRAVE,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TODECK,nil,1,tp,LOCATION_GRAVE)
end
function s.applyop(e,tp,eg,ep,ev,re,r,rp)
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EFFECT)
    local g=Duel.SelectMatchingCard(tp,s.applyfilter,tp,LOCATION_GRAVE,0,1,1,nil)
    local tc=g:GetFirst()
