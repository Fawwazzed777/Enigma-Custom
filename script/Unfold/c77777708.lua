...
function c77777708.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c77777708.cfilter,tp,LOCATION_PZONE,0,1,nil) and Duel.IsExistingMatchingCard(c77777708.filter,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c77777708.operation(e,tp,eg,ep,ev,re,r,rp)
  Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectMatchingCard(tp,c77777708.cfilter,tp,LOCATION_PZONE,0,1,1,nil)
	if Duel.Destroy(g,REASON_COST)~=0 then
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
    local g2=Duel.SelectMatchingCard(tp,c77777708.filter,tp,LOCATION_DECK,0,1,1,nil)
