...
function c77777717.pentg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local tc2=Duel.GetFieldCard(tp,LOCATION_PZONE,1)
	if chk==0 then
    if tc1 and tc1~=e:GetHandler() then return tc1:IsDestructable() and Duel.IsExistingMatchingCard(c77777717.penfilter,tp,LOCATION_DECK,0,1,nil) 
    elseif tc2 and tc2~=e:GetHandler() then return tc2:IsDestructable() and Duel.IsExistingMatchingCard(c77777717.thfilter,tp,LOCATION_DECK,0,1,nil) 
...
function c77777717.operation(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not c:IsRelateToEffect(e) then return end
	if Duel.Damage(1-tp,500,REASON_EFFECT)~=0 and Duel.IsExistingMatchingCard(Card.IsDestructable(),tp,0,LOCATION_ONFIELD,1,nil)then
    Duel.BreakEffect()
