--Rank-Up-Magic Darkest Phantasm
local s,id=GetID()
...
function s.filter1(c,e,tp)
	...
	return #pg<=1 and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
...
function s.filter2(c,e,tp,mc,rk,pg)
	...
	return c:IsType(TYPE_XYZ) and (c:IsSetCard(0x344) or c:IsSetCard(0x145)) and mc:IsType(TYPE_XYZ,c,SUMMON_TYPE_XYZ,tp) and c:IsRank(rk)
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE+LOCATION_REMOVED) and s.filter1(chkc,e,tp) end
