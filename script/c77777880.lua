...
function c77777880.ssfilter(c,e,tp)
	return c:IsSetCard(0x40c) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEDOWN_DEFENSE)
end
function c77777880.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsAbleToHand() and Duel.IsExistingMatchingCard(c77777880.ssfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp)  and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,0,0)
end
function c77777880.thop(e,tp,eg,ep,ev,re,r,rp)
  if Duel.SendtoHand(e:GetHandler(),nil,REASON_EFFECT)~=0 
  and Duel.IsExistingMatchingCard(c77777880.ssfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp)  and Duel.GetLocationCount(tp,LOCATION_MZONE)>1 then
