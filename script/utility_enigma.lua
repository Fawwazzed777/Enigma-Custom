...
-- Perbaikan Rescon agar perhitungan sum lebih stabil
function Vortex.Rescon(total_val,recipe)
    return function(sg,e,tp,mg)
        ...
        local res= Duel.GetLocationCountFromEx(tp,tp,sg,e:GetHandler())> 0
...
function Vortex.Target(total_val, recipe)
    return function(e,tp,eg,ep,ev,re,r,rp,chk,c)
        ...
        local sg=aux.SelectUnselectGroup(rg,e,tp,2,99,Vortex.Rescon(total_val,recipe),1,tp,HINTMSG_SPSUMMON,nil,nil,true)
        if sg and #sg > 0 then
            sg:KeepAlive()
            e:SetLabelObject(sg)
            return true
