...
function c77777721.initial_effect(c)
	...
	e2:SetCountLimit(1,77777728)
	e2:SetCode(EVENT_FREE_CHAIN)
	e2:SetRange(LOCATION_MZONE)
	e2:SetCost(c77777721.rmvcost)
  e2:SetTarget(c77777721.rmtg)
	...
	e6:SetCountLimit(1,77777721+EFFECT_COUNT_CODE_OATH)
...
function c77777721.sptg2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c77777721.spfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
...
function c77777721.spop2(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c77777721.spfilter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
...
function c77777721.filter(c)
	return (c:IsSetCard(0x95)or c:IsSetCard(0x5D2) or c:IsCode(71345905) or c:IsCode(511000225)) and c:IsAbleToHand()
end
function c77777721.rmtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsAbleToRemove,0,LOCATION_ONFIELD,0,1,nil) and Duel.IsExistingMatchingCard(c77777721.filter,tp,LOCATION_DECK,0,1,nil)end
