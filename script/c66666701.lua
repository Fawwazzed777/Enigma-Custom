...
function c66666701.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) then
		if Duel.SpecialSummon(tc,0,tp,tp,false,false,POS_FACEUP)~=0
			and Duel.IsExistingMatchingCard(c66666701.setfilter,tp,0,LOCATION_GRAVE,1,nil,tp)
      and Duel.SelectYesNo(tp,aux.Stringid(66666701,2))then
      Duel.BreakEffect()
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SET)
			local g=Duel.SelectMatchingCard(tp,c66666701.setfilter,tp,0,LOCATION_GRAVE,1,1,nil,tp)
			if g:GetCount()>0 and (g:GetFirst():IsType(TYPE_FIELD) or Duel.GetLocationCount(tp,LOCATION_SZONE)>0) then
        local tc=g:GetFirst()
				Duel.SSet(tp,tc)
