...
function s.initial_effect(c)
	...
	--Debuff 1600 ATK/DEF (Quick Effect)
	...
	c:RegisterEffect(e3)
	--Unaffected by monster effect while it has no material, except "Enigmation" or "Imaginary Force" monster.
	local ei=Effect.CreateEffect(c)
	ei:SetType(EFFECT_TYPE_SINGLE)
	ei:SetProperty(EFFECT_FLAG_SINGLE_RANGE)
	ei:SetRange(LOCATION_MZONE)
	ei:SetCode(EFFECT_IMMUNE_EFFECT)
	ei:SetValue(s.efilter)
	c:RegisterEffect(ei)
end
...
function s.efilter(e,te)
	local c=e:GetHandler()
	return c:GetOverlayCount()==0 and te:IsMonsterEffect() and te:GetOwner()~=e:GetOwner() and not (te:IsCardSetcode(0x344) or te:IsCardSetcode(0x303))
end