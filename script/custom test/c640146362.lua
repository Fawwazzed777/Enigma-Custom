...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
    ...
    if tc1 and Duel.SendtoDeck(tc1,nil,2,REASON_EFFECT)>0 and tc1:IsLocation(LOCATION_DECK+LOCATION_EXTRA) then
        ...
        elseif code==96488215 then
            Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
            local sg=Duel.SelectMatchingCard(tp,s.sumfilter,tp,0,LOCATION_MZONE,1,1,nil)
            local sc=sg:GetFirst()
            if sc then
                Duel.HintSelection(sg)
                local e1=Effect.CreateEffect(h)
                e1:SetType(EFFECT_TYPE_SINGLE) e1:SetCode(EFFECT_SET_ATTACK_FINAL) e1:SetValue(0)
                e1:SetReset(RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END)
                sc:RegisterEffect(e1)
                local e2=e1:Clone() e2:SetCode(EFFECT_SET_DEFENSE_FINAL)
                sc:RegisterEffect(e2)
            end			
        --OVER BURST DRAGON 
        elseif code==96488219 then
            ...
            for sc in aux.Next(sg) do
                ...
                e1:SetReset(RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END)
                sc:RegisterEffect(e1)
                local e2=e1:Clone() e2:SetCode(EFFECT_SET_ATTACK_FINAL) e2:SetValue(0)
                sc:RegisterEffect(e2)
                local e3=e1:Clone() e3:SetCode(EFFECT_SET_DEFENSE_FINAL)
                sc:RegisterEffect(e3)
            end
