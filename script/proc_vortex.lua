...
function Vortex.AddProcedure(c,total_val,recipe)
    ...
    ex:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE+EFFECT_FLAG_SET_AVAILABLE)
    ex:SetCode(EFFECT_CHANGE_RANK)
    ex:SetValue(function(e,tc)return e:GetHandler():GetLevel()end)
    c:RegisterEffect(ex)
	local ey=Effect.CreateEffect(c)
	ey:SetType(EFFECT_TYPE_SINGLE)
	ey:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE+EFFECT_FLAG_SET_AVAILABLE)
	ey:SetCode(EFFECT_ADD_TYPE)
	ey:SetValue(TYPE_XYZ)
	c:RegisterEffect(ey)
	local ez=Effect.CreateEffect(c)
    ez:SetType(EFFECT_TYPE_SINGLE)
	ez:SetProperty(EFFECT_FLAG_SINGLE_RANGE+EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE+EFFECT_FLAG_SET_AVAILABLE)
    ez:SetCode(EFFECT_RANK_LEVEL)
    ez:SetValue(1)
    c:RegisterEffect(ez)
